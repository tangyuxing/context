function [rcnn_model, rcnn_k_fold_model] = ...
    rcnn_train_context(imdb, varargin)
...
try
  ...
  [X_pos_feat, X_pos_context, keys_pos] = ...
      get_positive_pool5_context_features(imdb, opts);
% Do NOT save files
%   save(save_file, 'X_pos', 'keys_pos', '-v7.3');
end
% Init training caches
caches = cell(1, length(imdb.class_ids));
X_pos = cell(1, length(imdb.class_ids));
for i = imdb.class_ids
  fprintf('%14s has %6d positive instances\n', ...
      imdb.classes{i}, size(X_pos_feat{i},1));
  X_pos_feat{i} = rcnn_pool5_to_fcX(X_pos_feat{i}, opts.layer, rcnn_model);
  X_pos_feat{i} = rcnn_scale_features(X_pos_feat{i}, opts.feat_norm_mean);
  X_pos_context{i} = rcnn_scale_features(X_pos_context{i}, opts.context_norm_mean);
  % concatenate context with existing features
  X_pos{i} = cat(2, X_pos_feat{i}, X_pos_context{i});
  X_pos_feat{i} = [];
  X_pos_context{i} = [];
...
d_feat = rcnn_load_cached_pool5_features(opts.cache_name, ...
    imdb.name, imdb.image_ids{ind});
d_context = rcnn_load_cached_context_features(opts.cache_name, ...
    imdb.name, imdb.image_ids{ind});

class_ids = imdb.class_ids;

if isempty(d_feat.feat)
  X_neg = cell(max(class_ids), 1);
  keys = cell(max(class_ids), 1);
  return;
end

d_feat.feat = rcnn_pool5_to_fcX(d_feat.feat, opts.layer, rcnn_model);
d_feat.feat = rcnn_scale_features(d_feat.feat, opts.feat_norm_mean);
d_context.context = rcnn_scale_features(d_context.context, opts.context_norm_mean);
d_feat.feat = cat(2, d_feat.feat, d_context.context);

neg_ovr_thresh = 0.3;

if first_time
  for cls_id = class_ids
    I = find(d_feat.overlap(:, cls_id) < neg_ovr_thresh);
    X_neg{cls_id} = d_feat.feat(I,:);
    keys{cls_id} = [ind*ones(length(I),1) I];
  end
else
  zs = bsxfun(@plus, d_feat.feat*rcnn_model.detectors.W, rcnn_model.detectors.B);
  for cls_id = class_ids
    z = zs(:, cls_id);
    I = find((z > caches{cls_id}.hard_thresh) & ...
             (d_feat.overlap(:, cls_id) < neg_ovr_thresh));
...
% ------------------------------------------------------------------------
function [X_pos_feat, X_pos_context, keys] = get_positive_pool5_context_features(imdb, opts)
% ------------------------------------------------------------------------
X_pos_feat = cell(max(imdb.class_ids), 1);
X_pos_context = cell(max(imdb.class_ids), 1);
...
for i = 1:length(imdb.image_ids)
  ...
  d_context = rcnn_load_cached_context_features(opts.cache_name, ...
      imdb.name, imdb.image_ids{i});
  for j = imdb.class_ids
    if isempty(X_pos_feat{j})
      X_pos_feat{j} = single([]);
      X_pos_context{j} = single([]);
      keys{j} = [];
    end
    sel = find(d_feat.class == j);
    if ~isempty(sel)
      X_pos_feat{j} = cat(1, X_pos_feat{j}, d_feat.feat(sel,:));
      X_pos_context{j} = cat(1, X_pos_context{j}, d_context.context(sel,:));
