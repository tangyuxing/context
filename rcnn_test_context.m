function res = rcnn_test_context(rcnn_model, imdb, suffix)
...
try
  ...
  for f = 1:length(folds)
    for i = folds{f}
      ...
      d_feat = rcnn_load_cached_pool5_features(feat_opts.cache_name, ...
          imdb.name, image_ids{i});
      d_context = rcnn_load_cached_context_features(feat_opts.cache_name, ...
          imdb.name, image_ids{i});
      if isempty(d_feat.feat)
        continue;
      end
      d_feat.feat = rcnn_pool5_to_fcX(d_feat.feat, feat_opts.layer, rcnn_model);
      d_feat.feat = rcnn_scale_features(d_feat.feat, feat_opts.feat_norm_mean);
      d_context.context = rcnn_scale_features(d_context.context, feat_opts.context_norm_mean);
      d_feat.feat = cat(2, d_feat.feat, d_context.context);
      zs = bsxfun(@plus, d_feat.feat*rcnn_model.detectors(f).W, rcnn_model.detectors(f).B);

      for j = 1:num_classes
        boxes = d_feat.boxes;
        z = zs(:,j);
        I = find(~d_feat.gt & z > thresh(j));
        boxes = boxes(I,:);
        scores = z(I);
