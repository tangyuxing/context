...
d = rcnn_load_cached_context_features(opts.cache_name, ...
...
if first_time
  ...
  zs = bsxfun(@plus, d.context*rcnn_model.detectors.W, rcnn_model.detectors.B);
  for cls_id = class_ids
    z = zs(:, cls_id);
...
for i = 1:length(imdb.image_ids)
  ...
  for j = imdb.class_ids
    ...
    sel = find(d.class == j);
    if ~isempty(sel)
      X_pos{j} = cat(1, X_pos{j}, d.context(sel,:));
